...
// Goal: up to 500m of depth,for a maximum cost of $500
...
// Aug 2022 - tests in Menorca and release of design, assembly guide, 3D parts and code in GitHub.
...
// T2 - Arduino connects to GoPro. T2 = T1 + 6 seconds
// T3 - turn magnetic switch (MS) on > loop programs starts and wakes up GoPro. T3 = T2 + max 3 minutes
...
const int oneMinute = 5000;       // 60,000 millisecons = 1 minute NEED TO CHANGE TO 60000

                                  // settings for the mission
int dropTime = 2;                 // estimated duration to get to the seabed in minutes
                                  // it CAN'T be more than 5m. GoPro turns into sleep mode after 5m
int recordingTime = 3;            // duration of video recording in minutes
...
void setup() {
  ...
  printInLog("Hello! Welcome to the blue_eye 1.0");
...
for(int i=0; i<5; i++){                   // While testing in DC, with no magnetic switch
...
// coding for burn wire PENDING
void BurnWire(){

  printInLog("Burning wire");

  // turns burnwire in order to release dropcam
